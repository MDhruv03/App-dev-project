...
Discovering relevant opportunities. Tracking applications and deadlines. Preparing for interviews.
...
Opportunity platforms focus on listing and search but provide limited structured application tracking. Tracking tools generally do not include intelligent recommendation or profile-aware ranking. Interview preparation tools are detached from actual application pipeline state. Many solutions do not provide robust offline-first behavior for unstable connectivity.
...
Build a unified mobile workflow for discovering, saving, applying, and tracking opportunities. Design and implement a profile-aware recommendation mechanism for ranking opportunities. Provide interview preparation through question banks, answer evaluation, readiness metrics, and live interview simulation. Support reliability through local persistence, background synchronization, and fallback mechanisms. Provide analytics and export features for user decision support.

\subsection{Research Contribution}

This work contributes an integrated, production-style Android architecture that combines:

Multi-factor opportunity recommendation. Offline-first data handling with queued sync operations. AI-assisted and local fallback interview evaluation. Live voice interview interaction with speech and optional Polly-based synthesis. Application analytics and CSV export for user-level review.
...
\textbf{Methodological Gap:} Existing tools optimize isolated modules (listings, tracking, or interview prep), not a complete pipeline. \textbf{Practical-Knowledge Gap:} Users need one mobile workflow that transitions from discovery to interview execution without context switching. \textbf{Evidence Gap:} Offline-first behavior with queue-based sync and retry strategies is underemphasized in student career tools. \textbf{Usability Gap:} Voice-interactive interview rehearsal with fallback behavior is uncommon in mainstream student-focused apps.
...
Develop an Android platform that centralizes internships, jobs, and hackathons. Rank opportunities using profile-aware recommendation scoring. Provide searchable and filterable lists with save/apply actions. Track application statuses across pipeline states (saved, applied, interview, rejected, accepted). Build an AI interview module with domain selection, scoring, and readiness tracking. Provide a live interview mode with speech recognition, speaking pace estimation, follow-up prompts, and interviewer voice output. Ensure reliability with Room persistence, periodic workers, and pending sync operation retry logic. Provide analytics and export capabilities for user review.

\section{Design And Methodology}

\subsection{Software Requirements (SR)}

Android Studio and Gradle-based Android build pipeline. Java 17 and Kotlin interoperability. Android SDK (minSdk 24, target/compile SDK configured in project). Room database for local persistence. Retrofit and OkHttp for API integration. WorkManager for periodic sync and notifications. Compose + Material 3 UI components. CameraX and Android Speech APIs for live interview mode.

\subsection{System Architecture}

The project follows MVVM with a repository abstraction:

\textbf{View Layer:} Compose-based activities/fragments for Home, Saved, Tracker, AI Interview, Roadmap, Analytics, and Profile. \textbf{ViewModel Layer:} State management, filtering, scoring injection, and UI orchestration. \textbf{Repository Layer:} Network-first retrieval with local fallback, write-through updates, and merge logic. \textbf{Persistence Layer:} Room entities for opportunities, applications, interview data, user profile, and pending sync operations. \textbf{Worker Layer:} Periodic sync, daily recommendation notifications, and deadline reminders.
...
Role/type match. Location match. Paid preference match. Skill matches. Remote preference. Deadline proximity (in preference-mode path).

Score-derived outputs:

Recommendation ranking values stored per opportunity. Match percentage injected for UI display. Top-N selection used for recommended sections and notifications.

\subsection{Interview Evaluation Methodology}

The interview pipeline supports two evaluation modes:

\textbf{AI Evaluation Mode:} Uses remote evaluation client for score, feedback, strengths, improvements, and verdict. \textbf{Local Fallback Mode:} Keyword overlap and answer quality heuristics when AI evaluation is unavailable.

Live interview mode includes:

Speech recognition with partial/final transcript handling. Non-answer retry strategy with controlled thresholds. Follow-up question generation when response depth is insufficient. Speaking pace estimate using words-per-minute. Voice output using Polly proxy when configured, with automatic TTS fallback.

\subsection{Offline Sync and Reliability Methodology}

Application writes are attempted online first when possible. On offline/error conditions, writes persist locally and enqueue pending operations. Pending operations maintain retry metadata and error context. Background sync attempts replay operations in batches and removes successful entries. Sync workers and task schedulers provide periodic recovery.
...
\begin{figure*}[!tb]
\centering
\IfFileExists{build/report_assets/figure_1.png}{%
  \includegraphics[width=0.98\textwidth]{build/report_assets/figure_1.png}%
}{%
  \fbox{\parbox{0.9\textwidth}{Missing figure file: build/report\_assets/figure\_1.png}}%
}
...
\begin{figure*}[!tb]
\centering
\IfFileExists{build/report_assets/figure_2.png}{%
  \includegraphics[width=0.98\textwidth]{build/report_assets/figure_2.png}%
}{%
  \fbox{\parbox{0.9\textwidth}{Missing figure file: build/report\_assets/figure\_2.png}}%
}
...
\begin{figure*}[!tb]
\centering
\IfFileExists{build/report_assets/figure_3.png}{%
  \includegraphics[width=0.98\textwidth]{build/report_assets/figure_3.png}%
}{%
  \fbox{\parbox{0.9\textwidth}{Missing figure file: build/report\_assets/figure\_3.png}}%
}
...
\begin{figure*}[!tb]
\centering
\IfFileExists{build/report_assets/figure_4.png}{%
  \includegraphics[width=0.98\textwidth]{build/report_assets/figure_4.png}%
}{%
  \fbox{\parbox{0.9\textwidth}{Missing figure file: build/report\_assets/figure\_4.png}}%
}
...
\begin{figure*}[!tb]
\centering
\IfFileExists{build/report_assets/figure_5.png}{%
  \includegraphics[width=0.98\textwidth]{build/report_assets/figure_5.png}%
}{%
  \fbox{\parbox{0.9\textwidth}{Missing figure file: build/report\_assets/figure\_5.png}}%
}
...
\textbf{Career Accessibility:} Consolidates fragmented workflows, reducing cognitive and time burden for students and early professionals. \textbf{Skill Development:} Encourages structured interview practice with actionable feedback loops. \textbf{Inclusion Through Mobile-first Design:} Supports day-to-day use on common Android devices and intermittent connectivity. \textbf{Reduced Operational Waste:} Digital application tracking and reminders reduce dependence on ad-hoc manual note-taking and repeated effort. \textbf{Scalable Educational Utility:} The architecture can be reused in placement cells and institutional career support ecosystems.
...
\textbf{Presentation Layer:} Compose screens for main workflows and interactions. \textbf{Domain/Logic Layer:} Recommendation and interview logic components. \textbf{Data Layer:} Repositories coordinating local+remote data strategy. \textbf{Persistence and Sync Layer:} Room entities, pending operation queues, periodic workers. \textbf{Device Integration Layer:} Speech recognition, TextToSpeech, CameraX, notification channels.

\section{Use Of Advanced Concepts}

MVVM architecture with reactive LiveData state propagation. Compose Material 3 UI with multi-screen state handling. Offline-first synchronization through queued pending operations and retry policies. WorkManager periodic jobs for sync and recommendations. Multi-factor recommendation and profile-aware scoring. Local and AI hybrid interview evaluation strategy. Live interview voice loop combining STT, adaptive prompting, and TTS/Polly output. CSV export for analytics portability. CameraX integration for real-time interview preview. SharedPreferences-driven personalization (theme, roadmap completion).
...
Recommendation output supports top-ranked subsets for UI and notifications. Local fallback seeding inserts 60 opportunities when no cache exists. Live interview plan uses a multi-turn structure (intro, technical set, probe, behavioral, closing). Sync system includes pending operation batching and retry control. Unit-test files exist for sync helper logic (application/opportunity/interview merge helpers).
...
Build reproducibility currently depends on correcting machine-specific local SDK/JDK paths. Resume parsing from raw PDF bytes is keyword-based and may miss structured PDF semantics. Some data flows still rely on mock API service behavior and need production API hardening. Default single-user assumptions (for example, fixed mock user ID) limit multi-user realism. Full-scale instrumented testing on diverse devices/emulators was constrained by environment configuration.

\section{Future Scope}

Integrate production opportunity APIs and authenticated user accounts. Add stronger resume parsing using robust document parsing pipelines. Extend recommendation with learning-to-rank, implicit feedback, and temporal modeling. Add richer interview analytics (topic weakness trajectories, confidence bands, adaptive sequencing). Expand roadmap generation with milestone deadlines and mentor feedback loops. Add CI/CD pipeline for automated unit/instrumented test execution. Improve privacy/security with encrypted local storage and finer-grained sync conflict strategies.
